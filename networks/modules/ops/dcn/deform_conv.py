...
class ModulatedDeformConvPack(ModulatedDeformConv):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.conv_offset = nn.Conv2d(
            ...
            # deformable_groups 表示将输入通道分成多少组可变形的卷积核
            # # 每组可变形卷积核共享相同的偏移和调制参数
            # # 对输入特征图的不同区域进行卷积操作，每组专注于输入特征图的不同区域，可以增强模型的感受野。
            # ×3 的含义
            # # 1个调制系数（modulation factor）
            # # 2个偏移量（分别对应x和y方向）
            kernel_size=self.kernel_size,
            # 卷积核通常都是3x3（H=W）所以这里不用_pair函数
            stride=_pair(self.stride), # 转为一对元组 (stride, stride)
            padding=_pair(self.padding), # 转为一对元组 (padding, padding)
            dilation=_pair(self.dilation), # 转为一对元组 (dilation, dilation)
            bias=True)
        self.init_weights()
    ...
    def forward(self, x):
        out = self.conv_offset(x) # shape (N, 3*kernel_size*kernel_size*deformable_groups, H, W)
        o1, o2, mask = torch.chunk(out, 3, dim=1) # 分别得到偏移量、调制系数和mask
        offset = torch.cat((o1, o2), dim=1) # 将偏移量、调制系数拼接起来
        mask = torch.sigmoid(mask) # 对调制系数进行sigmoid激活
        return modulated_deform_conv(x, offset, mask, 
                                     self.weight, self.bias,
                                     self.stride, self.padding, self.dilation,
                                     self.groups, self.deformable_groups) # 卷积操作
