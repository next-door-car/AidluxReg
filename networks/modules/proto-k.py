...
class ProtoMemory(nn.Module):
    ...
    def pool_ap_init(self, seed=0, gpu_num=0, temperature=1):
        ...
        if self.pool_structured.item() < 1.0:
            # AP阶段=>未被结构化 
    ...
    def forward(self, x, evaluation=False):
        ...
        if self._get_warmup_state(): # 没有达到warmup状态
            ...
            with torch.no_grad():
                ...
                # 一定要做归一化
                theta_atten_proto_normalized = F.normalize(theta_atten_proto, p=2, dim=1) 
                ...
                cluster_affinity = F.softmax(theta_atten_proto_normalized, dim=1)  # n * h * w, num_k
                ## print(f"cluster_affinity.max(1) {cluster_affinity.max(1).values} {cluster_affinity.max(1).indices}")
            # # 固定值应收缩 
            ...
            # 自适应收缩限制 => 概率论
            self.sorted_bins = cluster_affinity.size(1) # 必须比 cluster_affinity.size(1)小,否则index=0
            self.shrink_thres_percentage = 0.25 # 靠前的百分之80
            shrinked_cluster_affinity = torch.zeros_like(cluster_affinity)  # 创建一个与cluster_affinity相同形状的张量来存储收缩后的值
            if self.shrink_thres_percentage > 0:
                # hard shrink 
                sorted_affinity, _ = torch.sort(cluster_affinity, dim=1, descending=False)  # 对每个特征的相似度进行降序排列
                histograms = torch.stack([torch.histc(sorted_affinity[i], bins=self.sorted_bins, 
                ...
                threshold_index = threshold_indices * (sorted_affinity.size(1) // self.sorted_bins)
            ...
            beta_residual = 0.9*concept_residuals + 0.1*context_residuals
            ...
            if self.training:
                ...
                with torch.no_grad():
                    ...
                    ## max(1) 返回两个值：第一个是最大值本身概率，第二个是最大值的索引。
