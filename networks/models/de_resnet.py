...
class UpsampleConv2d(nn.Module):
    ...
    def __init__(self, 
                 ...
                 kernel, stride): # kernel 尽量保持为奇数，padding才能/2
        super().__init__()
        self.input_filters = input_filters
        self.output_filters = output_filters
        self.kernel = kernel
        self.strides = stride
        self.conv_layer = nn.Conv2d(self.input_filters, self.output_filters, 
                                    self.kernel, self.strides, padding=int((self.kernel-1)/2)) # 256-3+2 / 2 + 1 =255/2+1 = 127+1 = 128

    def forward(self, x):
        height, width = x.size(2), x.size(3)
        new_height = height * (self.strides**2) # 64=>256
        new_width = width * (self.strides**2) # 64=>256

        x_resized = F.interpolate(x, size=(new_height, new_width), mode='nearest') # 插值再卷积

        return self.conv_layer(x_resized)
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.inplanes = 512 * block.expansion # 256 * block.expansion
        self.dilation = 1
        ...
        self.layer1 = self._make_layer(block, 256, layers[0], stride=2) # 原本无ocbe时候，stride = 1
        ...
        # # 定义转置卷积层，用于上采样
        # self.relu = nn.ReLU(inplace=True)
        # self.deconv3x3 = nn.ConvTranspose2d(64*block.expansion, 64, 
        #                                     kernel_size=3, stride=2,    
        #                                     padding=1, output_padding=1, bias=False)
        # self.bn3x3 = norm_layer(64) 
        # ## (64-1)*stride-2*padding+kernel_size+output_padding = 126-2+3+1 = 128
        # self.deconv7x7 = nn.ConvTranspose2d(64, 3, 
        #                                     kernel_size=7, stride=2,    
        #                                     padding=3, output_padding=1, bias=False) 
        # self.bn7x7 = norm_layer(3) 
        # ## (128-1)*stride-2*padding+kernel_size+output_padding = 254-6+7+1 = 256
        # ## 我们希望上采样到原始尺寸的两倍，因此 stride=2
        # ## kernel_size 通常选择为 4，以便能够实现 2x 的上采样
        # ## padding 通常设置为 kernel_size - stride，以确保输出尺寸
